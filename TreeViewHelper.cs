...
namespace ScenarioEditor
{
        ...
        where R : IUnit
    {
        ...
        public void ExpandAndMark(IList<T> echelons) {
             //unmark unused items — restore the XAML binding by clearing the local override
        ...
        public void MouseMove(object sender, MouseEventArgs e)
        {
            ...
            bool isRight = e.RightButton == MouseButtonState.Pressed;

            if ((isLeft || isRight) && treeView.SelectedValue != null)
            {
                Point currentPosition = e.GetPosition(treeView);
                ...
                if ((Math.Abs(currentPosition.X - _lastMouseDown.X) > 10.0) ||
                    (Math.Abs(currentPosition.Y - _lastMouseDown.Y) > 10.0))
                {
                    _lastDragEffect = DragDropEffects.None;
                    DataObject dragData = new DataObject();
                    dragData.SetData(sharedEchelonSelection.GetType(), sharedEchelonSelection);
                    DragDropState.WithChildren = isRight;
                    treeView.GiveFeedback += OnGiveFeedback;
                    DragDrop.DoDragDrop(treeView, dragData, DragDropEffects.All);
                    treeView.GiveFeedback -= OnGiveFeedback;
                    DragDropState.WithChildren = false;
                }
            }
        }
        ...
        int _dragTicks = 0;
        DragDropEffects _lastDragEffect = DragDropEffects.None;
        ...
        public void DragOver(object sender, DragEventArgs e)
        {
                    ...
                    (Math.Abs(currentPosition.Y - _lastMouseDown.Y) > 10.0))
            {
                ...
                if (targetItem == null)
                {
                    // Cross-roster OOB→Scenario drag onto blank space is always valid
                    // (PlaceDataObjectOnScenario handles it). Force Move so WPF doesn't
                    // suppress the Drop event before _lastDragEffect is ever set to Move.
                    if (isScenarioTree)
                    {
                        EchelonSelectionSet<OOBEchelon, OOBUnit> oobSel =
                            EchelonSelectionSet<OOBEchelon, OOBUnit>.ExtractFromDataObject(e.Data as DataObject);
                        if (oobSel != null)
                        {
                            _lastDragEffect = DragDropEffects.Move;
                            e.Effects = DragDropEffects.Move;
                            e.Handled = true;
                            return;
                        }
                    }
                    // Cursor is in the gap between tree items. Use the last evaluated effect
                    // rather than defaulting to Move — avoids a brief "OK" flash when dragging
                    // an incompatible unit across item boundaries.
                    e.Effects = _lastDragEffect;
                    e.Handled = true;
                    return;
                }
        ...
        {
            // Use the effect the current drop target reported — not _lastDragEffect,
            // which only reflects the last tree-item evaluation and would wrongly
            // show "No" when the cursor moves to a non-tree drop target (e.g. the map).
        ...
        public void Drop(object sender, DragEventArgs args)
        {
            ...
            else if (isScenarioTree)
            {
                    ...
                    PlaceDataObjectOnScenario(args.Data as DataObject, addChildren);
                    args.Effects = DragDropEffects.Move;
                    args.Handled = true;
        ...
        public void PlaceDataObjectOnScenario(DataObject data, bool addChildren)
        {
            EchelonSelectionSet<OOBEchelon, OOBUnit> oobSelection = EchelonSelectionSet<OOBEchelon, OOBUnit>.ExtractFromDataObject(data);
        ...
        public void PlaceDataObjectOnEchelon(DataObject data, T targetEchelon, bool addChildren)
        {
            ...
            bool wasActive = undoStack != null && undoStack.active;

            // Capture pre-drag state before suppressing undo, so the snapshot is
            // taken independently of whatever previousOOBState currently holds.
            NorbSoftDev.SOW.OOBState preDragState = wasActive ? undoStack.CaptureCurrentOOBState() : null;

            if (wasActive) undoStack.active = false;
            ...
            if (wasActive)
            {
                undoStack.oob_SaveBulkStateExplicit("oob_drag", preDragState);
        ...
        public void CollapseBelowRank(ERank rank, ItemsControl parentContainer)
        {
            foreach (Object item in parentContainer.Items)
            {
                ...
                {
                    currentContainer.IsExpanded = true;
                    // Do NOT call treeView.UpdateLayout() here — it forces a full layout pass
                    // for every node, causing O(n) layout passes. Containers already exist
                    // because all items start expanded via the TreeViewItem style.
                    CollapseBelowRank(rank, currentContainer);
                }
                else
                {
                    currentContainer.IsExpanded = false;
                    // No need to recurse into collapsed items — their children are hidden.
                }
            }
        }
