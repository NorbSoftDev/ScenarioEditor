...
namespace ScenarioEditor
{
        ...
        where R : EchelonGeneric<T>
    {
        ...
        public void Execute(object parameter)
        {
            ...
            // Capture a pre-delete CSV snapshot (does not push an undo action), perform the removal
            // while preventing intermediate undo events, then record a post-delete snapshot so
            // the undo stack contains a correct previous->current state for the bulk change.
            SOWScenarioEditorWindow.GetScenarioUndoStack().UpdateScenarioRosterState();
    ...
    public class AddToScenarioCommand : ICommand
    {
        public AddToScenarioCommand() { }

        public void Execute(object parameter)
        {
            if (parameter == null) return;
            OOBEchelon echelon = parameter as OOBEchelon;
            if (echelon == null) return;
            Scenario scenario = SOWScenarioEditorWindow.GetScenario();
            if (scenario == null) return;
            if (echelon.unit == null) return;
            try { scenario.InsertUnit(echelon.unit); }
            catch (Exception ex) { Log.Error(null, ex.Message); }
        }
        ...
        public bool CanExecute(object parameter)
        {
            if (parameter == null) return false;
            OOBEchelon echelon = parameter as OOBEchelon;
            if (echelon?.unit == null) return false;
            return SOWScenarioEditorWindow.GetScenario() != null;
        }
    ...
    public class AddToScenarioWithChildrenCommand : ICommand
    {
        public AddToScenarioWithChildrenCommand() { }
        ...
        public void Execute(object parameter)
        {
            ...
            Scenario scenario = SOWScenarioEditorWindow.GetScenario();
            if (scenario == null) return;
            scenario.InsertEchelonWithChildren(echelon);
        }
